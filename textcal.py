...
def calc(math_type):
    ...
    math = math_type
    first_num = disbox.get()
    clear()
def equal():
    result = ''
    second_num = disbox.get()
    clear()
    if math == 'add':
        ...
        result = int(first_num) / int( second_num)
    disbox.insert(0, str(result))

#creating widgets
disbox = Entry(mw,width = 14, font = ('Arial',28),justify = RIGHT)
...
disbox.grid(row = 0, column = 0,columnspan = 3,  padx = 10, pady = 10)
